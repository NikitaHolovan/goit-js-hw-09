...
function createPromise(pos, delay) {
  const shouldResolve = Math.random() > 0.3; //console.log(shouldResolve)
...
function onFormSubmit(e) {
  ...
  for (let pos = 1; pos <= amount; pos += 1) {
    createPromise(pos, delay).then(({
      pos,
      delay
    }) => {
      _notiflixNotifyAio.Notify.success(`Fulfilled promise ${pos} in ${delay}ms`);
      ...
      _notiflixNotifyAio.Notify.failure(`Rejected promise ${pos} in ${delay}ms`);
